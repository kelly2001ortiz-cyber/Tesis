...
# Modelo de Hognestad para hormigon no confinado
...
def resultantes_hormigon(Fibras, sigma_c, c, phi, fc0, ec0, esp):
    yi = Fibras[:, 0]
    Ai = Fibras[:, 1]
    ec = -phi * (yi - c)
...
def resultantes(c, phi, Fu, As, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, P):
    N_uc, M_uc, e_uc = resultantes_hormigon(Fu, hognestad, c, phi, fc0, ec0, esp)
...
def momrot(c_min, c_max, phi, Fu, As, tol, fc0, ec0, esp,
           fy, fsu, Es, ey, esh, esu, P, h, c_prev, phi_prev):

    def N_equilibrio(c):
        N = resultantes(c, phi, Fu, As, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, P)[0]
        return N
    ...
    M = resultantes(c, phi, Fu, As, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, P)[1]
    return M, c

# Funcion para obtener el doagrama momento curvatura
def diagrama_MC(Fu, As, h, tol, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, P):
    ...
    for _ in range(1000):
        Mi, ci = momrot(c_min, c_max, phi, Fu, As, tol, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu,
                        P, h, c_prev, phi_prev)
...
def ejecutar_mc_hognestad_columnaX (datos_hormigon, datos_acero, datos_seccion, datos_fibras):
    ...
    ny = int(float(datos_fibras.get("fibras_x")))
    nx = int(float(datos_fibras.get("fibras_y")))

    tol = 1e-3

    As = barras_columna(h, r, de, Nb1, Nb2, d_corner, d_edge)
    Fu = malla(b, h, r, de, ny)
    M, phi, _ = diagrama_MC(Fu, As, h, tol, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, P)
