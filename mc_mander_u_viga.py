...
def mander_u(ec, fc0, ec0, esp, Ec):
    ...
    zona2 = (ec > ec00) & (ec <= esp)
    fc[zona2] = fc0 * (2 * r) / (r - 1 + 2 ** r) * (1 - (ec[zona2] - ec00) / (esp - ec00))
    return fc

def resultantes_hormigon(Fibras, sigma_c, tan_theta, c, fc0, ec0, esp, Ec):
    y = Fibras[:, 0]
    a = Fibras[:, 1]
    ec = tan_theta * (y - c)
...
def resultantes(theta, Fu, As, c, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, Ec):
    tan_theta = np.tan(theta)
    N_UC, M_UC = resultantes_hormigon(Fu, mander_u, tan_theta, c, fc0, ec0, esp, Ec)
    es = tan_theta * (As[:, 1] - c)
...
def momrot(theta, Fu, As, h, tol, cmin, cmax, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, Ec):
    def equilibrio(c):
        N, _ = resultantes(theta, Fu, As, c, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, Ec)
        return N
    c = brentq(equilibrio, cmin, cmax, xtol=tol)
    _, M = resultantes(theta, Fu, As, c, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, Ec)
    return M, c

def diagrama_MC(Fu, As, h, tol, thetaf, m, fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, Ec):
    dtheta = thetaf / m
    ...
    M[0], c_sol[0] = momrot(thetas[0], Fu, As, h, tol, cmin_base, cmax_base,
                            fc0, ec0, esp, fy, fsu, Es, ey, esh, esu, Ec)
    for i in range(1, m):
        delta = h * delta_frac
