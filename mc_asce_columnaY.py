...
def calcular_respuesta_seccion(fc, fy, Ec, b, h, rec, d_est, s_est,
    ...
    if not isinstance(params, tuple) or len(params) != 3:
        # Propaga un error claro para que la UI lo muestre
...
def ejecutar_mc_asce_columnaY (datos_hormigon, datos_acero, datos_seccion, datos_asce, condicion):
    ...
    n_var_x = int(float(datos_seccion.get("disenar_columna_varillasX_2")))
    n_var_y = int(float(datos_seccion.get("disenar_columna_varillasY_2")))
