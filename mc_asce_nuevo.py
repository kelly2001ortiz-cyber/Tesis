...
def parametros_columna(rel_axial, rho_t, vy_vo, fc, fyt):
    rel_axial = max(0.0, float(rel_axial))  # debe llegar un valor posiivo
    rho_t = max(float(rho_t), 1e-6)         # evitar division por cero
    vy_vo = max(0.2, float(vy_vo))       # ASCE limita este valor a 0.2 como mínimo
    rho_t_eff = min(rho_t, 0.0175)       # ASCE limita el aporte efectivo de rho_t 
    
    a = max(0.0, 0.042 - 0.043 * rel_axial + 0.63 * rho_t_eff - 0.023 * vy_vo)  # parametro a

    rel_b = min(rel_axial, 0.5)
    den = 5.0 + (rel_b / 0.8) * (1.0 / rho_t_eff) * (fc / max(fyt, 1e-6))
    ...
    c = max(0.0, 0.24 - 0.4 * rel_axial)  # parametro c, independiente de confinamiento
