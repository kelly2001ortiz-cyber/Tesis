...
class dibujar_fibras(QWidget):
    ...
    def __init__(self, data, tipo, *args, **kwargs):
    ...
    def rejilla(self):
        ...
        # ---------------------------------------------------------
        if barras_2d is not None and len(barras_2d) > 0:
            radios = np.sqrt(barras_2d[:, 2] / np.pi)

            for (xs, ys, _), rb in zip(barras_2d, radios):
                self.ax.add_patch(
                    patches.Circle(
                        (xs, ys),
                        rb,
        ...
        self._C = np.vstack(centros_arrays) if centros_arrays else np.empty((0, 2))
        ...
        if self.conf:
            ...
            V = np.vstack((V, interseccion_nucleo, esquinas_nucleo))
        else:
            V = np.vstack((V))

        # Centro de barras longitudinales
        if barras_2d is not None and len(barras_2d) > 0:
            V = np.vstack((V, barras_2d[:, :2]))

        V = np.unique(np.round(V, 10), axis=0)
    ...
    def on_mouse_move(self, event):
        if not (event.inaxes and event.x is not None and event.y is not None):
            self.label.setText(" "); self.remove_highlight()
            return
